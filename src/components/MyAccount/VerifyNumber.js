...
const VerifyNumber = (props) => {
  ...
  const onVerifyPhone = async (otpVal) => {
    ...
    const response = await props.verifyPhone({
      code: otpVal,
    ...
    if (response && response.data) {
      if (response.status == 200) {
        ...
        showMessage("شماره موبایل شما با موفقیت تایید شد", "success");

        if (props.router.route === "/myaccount/profile") {
          props.userProfile();
          props.changeStatus(false);
        } else if (props.router.route === "/forget") {
          router.push(
            `/reset?code=${otpVal}&userId=${response.data.result.userId}`
          );
        }
      } else {
...
VerifyNumber.getInitialProps = async () => ({
...
const mapDispatchToProps = (dispatch) => ({
  ...
  verifyPhone: (params) => dispatch(verifyPhone(params, i18n.language)),
...
export default withTranslation("common")(
  connect(mapStateToProps, mapDispatchToProps)(withRouter(VerifyNumber))
);
