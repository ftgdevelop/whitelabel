import React, { useState ,useEffect} from 'react'
import PropTypes from 'prop-types'
import { i18n, withTranslation } from '../../../i18n'
import { useRouter } from 'next/router';
import { Col, Row, Button } from "antd"
import { GetReserveBus } from "../../actions/bus/bus"

import { BusIcon } from "../UI/Icons"
import Time from '../UI/Time/Time'
import DatePersion from '../UI/DatePersion/DatePersion'
import styles from '../../styles/Bus.module.css'

function numberWithCommas(x) {
  return x ? x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : x
}

const AsidePaymentBus = props => {
    const { t } = props;

    const router = useRouter();
    const reserveId = router.query.reserveId;
    const username = router.query.username;

    const [reserveInfo, setReserveInfo] = useState();
    
    useEffect(() => {    
        const fetchData = async () => {
            const response = await GetReserveBus(reserveId, username);
            if (response.data) {
                setReserveInfo(response.data.result);
                props.getPrice(response.data.result.totalPrice);
            }
          };
        fetchData()
    }, []);
    
    return (
        <div className={`${styles.asideCheckout} ${styles.asideCheckoutFlightForeign}`}>
            {reserveInfo && <>
                <div className={styles.topAside}>
                    <div className={styles.subject}>جزئیات اتوبوس</div>

                    <div className={styles.flightSelected}>
                    <div className={styles.date}>
                        <BusIcon />
                        <span>{`حرکت از ${reserveInfo.source.city.name || ''} (${
                        reserveInfo.source.name
                        }) به ${reserveInfo.destination.city.name || ''} (${
                        reserveInfo.destination.name
                        })`}</span>
                    </div>
                    <div className={styles.contentFlightSelected}>
                        <Row>
                        <Col xs={24} sm={4} md={4} lg={4} xl={4}>
                            <div className={styles.airLine}>
                            {reserveInfo.office.picture.path && (
                                <img
                                src={reserveInfo.office.picture.path}
                                alt={reserveInfo.office.picture.altAttribute}
                                />
                            )}
                            </div>
                        </Col>
                        <Col xs={9} sm={4} md={4} lg={4} xl={4}>
                            <div className={styles.origin}>
                            <div className={styles.textOrigin}>
                                {reserveInfo.source.city.name || '-'}
                            </div>
                            <div className={styles.timeOrigin}>
                                {reserveInfo.source.name || '-'}
                            </div>
                            </div>
                        </Col>
                        <Col xs={6} sm={12} md={12} lg={12} xl={12}>
                            <div className={styles.symbolFlight}>
                            <div className={styles.airlineType}>
                                <DatePersion date={reserveInfo.departureDateTime} />
                            </div>
                            <img
                                className={styles.lineBusIcon}
                                src="data:image/svg+xml;base64,"
                            />
                            <div className={styles.dashedFlight}></div>
                            <div className={styles.typeFlight}>
                                <span>
                                ساعت حرکت
                                <Time date={reserveInfo.departureDateTime} />
                                </span>
                            </div>
                            </div>
                        </Col>
                        <Col xs={9} sm={4} md={4} lg={4} xl={4}>
                            <div className={styles.destination}>
                            <div className={styles.textDestination}>
                                {reserveInfo.destination.city.name || '-'}
                            </div>
                            <div className={styles.timeDestination}>
                                {reserveInfo.destination.name || '-'}
                            </div>
                            </div>
                        </Col>
                        </Row>
                    </div>
                    </div>
                </div>
                <div className={styles.detailsAside}>
                    <div className={styles.detailsAside}>
                        <div className={styles.subject}>
                            <span>جزئیات مبلغ</span>
                        </div>
                        <div className={styles.contentDetailsAside}>
                            <div className={styles.passengerDetails}>
                                <span>{reserveInfo.passengerCount} نفر</span>
                                <b>{numberWithCommas(reserveInfo.salePrice)} ریال</b>
                            </div>
                            <div class={styles.pricePay}>
                                <span>مجموع</span>
                                <b>{numberWithCommas(reserveInfo.totalPrice)} ریال</b>
                            </div>
                        </div>
                    </div>
                </div>
            </>}
        </div>
    )
}

AsidePaymentBus.getInitialProps = async () => ({
    namespacesRequired: ['common'],
})
  
AsidePaymentBus.propTypes = {
t: PropTypes.func.isRequired,
}
  
export default withTranslation('common')(AsidePaymentBus)
