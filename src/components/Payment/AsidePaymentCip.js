...
import ModalCipDetails from "./ModalCipDetails";
...
const AsidePaymentCip = props => {
    ...
    useEffect (()=>{
        const fetchData = async () => {
            const response = await CipGetReserveId(reserveId,username);
            if (response.data) {
                setReserveInfo(response.data.result);
                props.getPrice(response.data.result.totalSalePrice);
            }
          };
        fetchData()
    },[]);
    ...
    return <div className={`${styles.asideCheckout} ${styles.cipAsideCheckout}`}>
                        ...
                        <div className={styles.detailReserveCip}>
                            <ul>
                                <li>
                                    <div>شماره پرواز</div>
                                    <div>
                                        {reserveInfo.flightNumber && reserveInfo.flightNumber.toEnglishDigit()}
                                    </div>
                                </li>
                                <li>
                                <div>نام ایرلاین</div>
            ...
            {reserveInfo.passengers.length > 0 &&
                <div className={styles.passengerBookingSummary}>
                    <div className={styles.subject}>مسافران</div>
                        {reserveInfo.passengers.map((item, index) =><div className={styles.passengerSummaryCip} key={index}>
                                {item.passengerType === "Adult" && <div>
                                    ...
                                    {item.service.items.map((itemService, keyService) =>
                                        <div key={keyService} className={styles.itemService}>
                                            {itemService.name}
                                        </div>)}
                                </div>}
            ...
            <div className={styles.additionalService}>
                ...
                {reserveInfo.transport.items.find(i => i.count > 0) ? 
                    <div className={styles.additionalService}>
                        ...
                        {reserveInfo.transport.items.map((item, index) => <div className={styles.content}>آدرس : {item.address}</div>)[0]}
                    </div>
                    : null}
...
AsidePaymentCip.getInitialProps = async () => ({
    namespacesRequired: ['common'],
})
  
AsidePaymentCip.propTypes = {
t: PropTypes.func.isRequired,
}
  
export default withTranslation('common')(AsidePaymentCip)
