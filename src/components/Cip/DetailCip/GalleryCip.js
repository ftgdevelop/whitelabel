import React, { useState, useEffect } from "react";
import PropTypes from "prop-types";
import { Spin, Skeleton, Row, Col } from "antd";
import { i18n, withTranslation } from "../../../../i18n";
import ReactBnbGallery from "react-bnb-gallery";

import styles from "../../../styles/Cip.module.css";

const GalleryCip = (props) => {
  const [galleryOpened, setGalleryOpened] = useState(false);
  const [visible, setVisible] = useState(false);
  const [currentImg, setCurrentImg] = useState(0);
  const { t } = props;

  const openGallery = (a) => {
    setCurrentImg(a);
    setGalleryOpened(true);
  };
  const closeGallery = () => {
    setGalleryOpened(false);
  };

  const showModal = () => {
    setVisible(true);
  };

  const handleCancel = () => {
    setVisible(false);
  };
  return props.cipAirPortDetail ? (
    <>
      {props.cipAirPortDetail.galleries &&
      props.cipAirPortDetail.galleries.length > 0 ? (
        <>
          {props.cipAirPortDetail.galleries.length > 5 ? (
            // if gallery has more than 5 pictures:
            <div className={styles.galleryCip}>
              <div className={styles.mosaicGallaryCip}>
                <div className={styles.colMosaic}>
                  <div className={styles.largeMosaic}>
                    <div className={styles.imageLargeMosaic}>
                      <img
                        src={props.cipAirPortDetail.galleries[0].path}
                        className="full-width"
                        alt={props.cipAirPortDetail.galleries[0].altAttribute}
                        onClick={() => openGallery(0)}
                      />
                    </div>
                  </div>
                  <div className={styles.xSmallMosaic}>
                    <div className={styles.imageXSmallMap} onClick={showModal}>
                      <span className={styles.textViewMap}>
                        فرودگاه بر روی نقشه
                      </span>
                      <img src="data:image/svg+xml;base64," />
                    </div>
                    <div className={styles.imageXSmallMosaic}>
                      <img
                        src={props.cipAirPortDetail.galleries[1].path}
                        className="full-width"
                        alt={props.cipAirPortDetail.galleries[1].altAttribute}
                      />
                    </div>
                    <div className={styles.imageXSmallMosaic}>
                      <img
                        src={props.cipAirPortDetail.galleries[2].path}
                        className="full-width"
                        alt={props.cipAirPortDetail.galleries[2].altAttribute}
                      />
                    </div>
                  </div>
                </div>
                {props.cipAirPortDetail.galleries.length > 5 && (
                  <div className={styles.colMosaic}>
                    <div className={styles.smallMosaic}>
                      <div className={styles.imageSmallMosaic}>
                        <img
                          src={props.cipAirPortDetail.galleries[3].path}
                          className="full-width"
                          alt={props.cipAirPortDetail.galleries[3].altAttribute}
                        />
                      </div>
                      <div className={styles.imageSmallMosaic}>
                        <img
                          src={props.cipAirPortDetail.galleries[4].path}
                          className="full-width"
                          alt={props.cipAirPortDetail.galleries[4].altAttribute}
                        />
                      </div>
                    </div>
                    <div className={styles.smallMosaic}>
                      <div className={styles.imageSmallMosaic}>
                        <img
                          src={props.cipAirPortDetail.galleries[5].path}
                          className="full-width"
                          alt={props.cipAirPortDetail.galleries[5].altAttribute}
                        />
                      </div>
                      <div className={styles.imageSmallMosaic}>
                        <img
                          src={props.cipAirPortDetail.galleries[6].path}
                          className="full-width"
                          alt={props.cipAirPortDetail.galleries[6].altAttribute}
                        />
                      </div>
                    </div>
                  </div>
                )}
              </div>
            </div>
          ) : props.cipAirPortDetail.galleries.length > 1 ? (
            // if gallery has 2,3,4 or 5 pictures:
            <div className={"padding-h-15"}>
              <Row gutter={[15, 15]} wrap={false}>
                {props.cipAirPortDetail.galleries.map((item) => (
                  <Col>
                    <img
                      src={item.path}
                      alt={item.altAttribute}
                      className="full-width"
                    />
                  </Col>
                ))}
              </Row>
            </div>
          ) : (
            // if gallery has only 1 picture:
            <img
              src={props.cipAirPortDetail.picture.path}
              alt={props.cipAirPortDetail.name}
            />
          )}
          <ReactBnbGallery
            show={galleryOpened}
            photos={props.cipAirPortDetail.galleries}
            onClose={closeGallery}
            showThumbnails={true}
            activePhotoIndex={currentImg}
          />
        </>
      ) : (
        // if gallery has no picture main picture of cipAirportDetail:
        props.cipAirPortDetail.picture.path && (
          <img
            src={props.cipAirPortDetail.picture.path}
            alt={props.cipAirPortDetail.name}
          />
        )
      )}
    </>
  ) : (
    <div className={styles.galleryCip}>
      <div className={styles.mosaicGallaryCip}>
        <div className={styles.colMosaic}>
          <div className={styles.largeMosaic}>
            <div className={styles.imageLargeMosaic}>
              <Skeleton.Image className={styles.imageLargeMosaicSkeleton} />
            </div>
          </div>
          <div className={styles.xSmallMosaic}>
            <div className={styles.imageXSmallMap}>
              <Skeleton.Image className={styles.imageXSmallMapSkeleton} />
            </div>
            <div className={styles.imageXSmallMosaic}>
              <Skeleton.Image className={styles.imageXSmallMosaicSkeleton} />
            </div>
            <div className={styles.imageXSmallMosaic}>
              <Skeleton.Image className={styles.imageXSmallMosaicSkeleton} />
            </div>
          </div>
        </div>
        <div className={styles.colMosaic}>
          <div className={styles.smallMosaic}>
            <div className={styles.imageSmallMosaic}>
              <Skeleton.Image className={styles.imageSmallMosaicSkeleton} />
            </div>
            <div className={styles.imageSmallMosaic}>
              <Skeleton.Image className={styles.imageSmallMosaicSkeleton} />
            </div>
          </div>
          <div className={styles.smallMosaic}>
            <div className={styles.imageSmallMosaic}>
              <Skeleton.Image className={styles.imageSmallMosaicSkeleton} />
            </div>
            <div className={styles.imageSmallMosaic}>
              <Skeleton.Image className={styles.imageSmallMosaicSkeleton} />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

GalleryCip.getInitialProps = async () => ({
  namespacesRequired: ["common"],
});

GalleryCip.propTypes = {
  t: PropTypes.func.isRequired,
};

export default withTranslation("common")(GalleryCip);
