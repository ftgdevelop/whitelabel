import React,{ useState,useEffect } from 'react'
import PropTypes from 'prop-types'
import { Spin, Skeleton ,Row,Col} from 'antd'
import { i18n, withTranslation } from '../../../../i18n'
import ReactBnbGallery from 'react-bnb-gallery'
import defaultBanner from '../../../assets/defaultHotel.svg';

import styles from '../../../styles/Cip.module.css'
import { GalleryIcon } from '../../UI/Icons'

const GalleryCipTravelo = props =>{
    const [galleryOpened,setGalleryOpened] = useState(false);
    const [visible,setVisible] = useState(false);
    const [currentImg,setCurrentImg] = useState(0);
    const { t } = props;

    const openGallery = (a) => {
        setCurrentImg(a);
        setGalleryOpened(true);
    }
    const closeGallery = () => {
        setGalleryOpened(false)
    }

    const showModal = () => {
        setVisible(true)
    };
    
    const handleCancel = () => {
        setVisible(false)
    };

    const photoArray = props.cipAirPortDetail?.galleries;
    
    let photos = [];
    if (photoArray){
        for(let i=0 ; i< photoArray.length ; i++){
            let newItem = {};
            newItem.photo = photoArray[i].path;
            newItem.caption = photoArray[i].titleAttribute;
            newItem.subcaption = photoArray[i].altAttribute;
            newItem.thumbnail = photoArray[i].path;
            photos.push(newItem);
        }
    }
    
    return (
        <div
            className={`${styles.gallaryHotelCip} ${process.env.THEME_NAME === "TRAVELO" && styles.gallaryHotelCipTravelo}`}
            id="anchorgalleryhotel"
            >
            <div                
                className={`${styles.fluidContainer} ${process.env.THEME_NAME === "TRAVELO" && styles.container}`}
                >
                {!!props.cipAirPortDetail &&
                    <>
                        {
                            (props.cipAirPortDetail.galleries && props.cipAirPortDetail.galleries.length > 5) ?
                                <div className={styles.mosaicGallaryHotel}>
                                    <div className={styles.colMosaic}>
                                        <div className={styles.largeMosaic} onContextMenu={(e) => e.preventDefault()}>
                                            <div
                                                className={styles.imageLargeMosaic}
                                                onClick={() => openGallery(0)}>
                                                <img src={ props.cipAirPortDetail.galleries[0].path } />
                                            </div>
                                        </div>
                                        {process.env.THEME_NAME === "TAJAWAL" &&
                                            <div className={styles.xSmallMosaic}>
                                                {
                                                    (props.mapInfo.lat === 0) && (props.mapInfo.lang === 0) ?
                                                        null :
                                                        <>
                                                            <div className={styles.imageXSmallMap} onClick={showModal}>
                                                                <span className={styles.textViewMap}>{t('show-hotel-on-map')}</span>
                                                                <img src="data:image/svg+xml;base64," />
                                                            </div>
                                                            <Modal
                                                                title={`${props.hotelDetails.HotelCategoryName} ${props.hotelDetails.HotelName} ${props.hotelDetails.CityName} ${t('on-the-map')}`}
                                                                open={visible}
                                                                onCancel={handleCancel}
                                                                footer={null}
                                                            >
                                                                <div className="mapModal">
                                                                    <MapWithNoSSR mapInfo={props.mapInfo} />
                                                                </div>
                                                            </Modal>
                                                        </>
                                                }
                                                <div className={styles.imageXSmallMosaic} onClick={() => openGallery(1)} onContextMenu={(e) => e.preventDefault()}>
                                                    <img src={props.cipAirPortDetail.galleries[1].path} />
                                                </div>
                                                <div className={styles.imageXSmallMosaic} onContextMenu={(e) => e.preventDefault()}>
                                                    <img src={props.cipAirPortDetail.galleries[2].path} onClick={() => openGallery(2)} />
                                                </div>
                                            </div>
                                        }
                                    </div>
                                    <div className={styles.colMosaic}>
                                        <div className={styles.smallMosaic}>
                                            <div className={styles.imageSmallMosaic} onClick={() => openGallery(process.env.THEME_NAME === "TRAVELO" ? 2 : 3)} onContextMenu={(e) => e.preventDefault()}>
                                                <img src={props.cipAirPortDetail.galleries[process.env.THEME_NAME === "TRAVELO" ? 2 : 3].path} />
                                            </div>
                                            <div className={styles.imageSmallMosaic} onClick={() => openGallery(process.env.THEME_NAME === "TRAVELO" ? 3 : 4)} onContextMenu={(e) => e.preventDefault()}>
                                                {process.env.THEME_NAME === "TAJAWAL" && topComment &&
                                                    <div className={styles.commentSmallMosaic}>
                                                        {parse(topComment)}
                                                    </div>
                                                }
                                                <img src={props.cipAirPortDetail.galleries[process.env.THEME_NAME === "TRAVELO" ? 3 : 4].path} />
                                            </div>
                                        </div>
                                        <div className={styles.smallMosaic}>
                                            <div className={styles.imageSmallMosaic} onClick={() => openGallery(process.env.THEME_NAME === "TRAVELO" ? 4 : 5)} onContextMenu={(e) => e.preventDefault()}>
                                                <img src={props.cipAirPortDetail.galleries[process.env.THEME_NAME === "TRAVELO" ? 4 : 5].path} />
                                            </div>
                                            <div className={styles.imageSmallMosaic} onClick={() => openGallery(process.env.THEME_NAME === "TRAVELO" ? 5 : 6)} onContextMenu={(e) => e.preventDefault()}>
                                                <button
                                                    onClick={() => openGallery(props.cipAirPortDetail.galleries[process.env.THEME_NAME === "TRAVELO" ? 5 : 6].path)}
                                                    className={`${styles.toggleGalleryHotel} ${process.env.THEME_NAME === "TRAVELO" && styles.toggleGalleryHotelTravelo}`}
                                                >
                                                    {process.env.THEME_NAME === "TRAVELO" && <GalleryIcon />}
                                                    عکس های بیشتر
                                                    {/* {`+${props.cipAirPortDetail.galleries.length} ${t("picture")}`} */}
                                                </button>
                                                <img src={props.cipAirPortDetail.galleries[process.env.THEME_NAME === "TRAVELO" ? 5 : 6].path} onContextMenu={(e) => e.preventDefault()} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                : (props.cipAirPortDetail.galleries && props.cipAirPortDetail.galleries.length > 3) ?
                                    <div className={styles.mosaicGallaryHotel}>
                                        <div className={styles.colMosaic}>
                                            <div className={styles.largeMosaic} onContextMenu={(e) => e.preventDefault()}>
                                                <div className={styles.imageLargeMosaic} onClick={() => openGallery(0)}>
                                                    <img src={photos[0].thumbnail} />
                                                </div>
                                            </div>
                                            {process.env.THEME_NAME === "TAJAWAL" &&
                                                <div className={styles.xSmallMosaic}>
                                                    {
                                                        (props.mapInfo.lat === 0) && (props.mapInfo.lang === 0) ?
                                                            null :
                                                            <>
                                                                <div className={styles.imageXSmallMap} onClick={showModal}>
                                                                    <span className={styles.textViewMap}>{t('show-hotel-on-map')}</span>
                                                                    <img src="data:image/svg+xml;base64," />
                                                                </div>
                                                                <Modal
                                                                    title={`${props.hotelDetails.HotelCategoryName} ${props.hotelDetails.HotelName} ${props.hotelDetails.CityName} ${t('on-the-map')}`}
                                                                    open={visible}
                                                                    onCancel={handleCancel}
                                                                    footer={null}
                                                                >
                                                                    <div className="mapModal">
                                                                        <MapWithNoSSR mapInfo={props.mapInfo} />
                                                                    </div>
                                                                </Modal>
                                                            </>
                                                    }
                                                    <div className={styles.imageXSmallMosaic} onClick={() => openGallery(1)} onContextMenu={(e) => e.preventDefault()}>
                                                        <img src={photos[1].path} />
                                                    </div>
                                                    <div className={styles.imageXSmallMosaic}>
                                                        <img src={photos[2].path} onClick={() => openGallery(2)} onContextMenu={(e) => e.preventDefault()} />
                                                    </div>
                                                </div>
                                            }
                                        </div>
                                        <div className={styles.colMosaic}>
                                            <div className={`${styles.smallMosaic}`}>
                                                <div className={`${styles.smallMosaic} ${styles.smallMosaicWidth}`}>
                                                    <div className={`${styles.imageSmallMosaic} ${styles.imageSmallMosaicThreeImageWidth}`} onClick={() => openGallery(1)}>
                                                        <img src={photos[1].thumbnail} />
                                                    </div>
                                                </div>
                                            </div>
                                            <div className={styles.smallMosaic}>
                                                <div className={`${styles.smallMosaic} ${styles.smallMosaicWidth}`}>
                                                    <div className={`${styles.imageSmallMosaic} ${styles.imageSmallMosaicThreeImageWidth}`} onClick={() => openGallery(2)}>
                                                        <button
                                                            onClick={() => openGallery(2)}
                                                            className={`${styles.toggleGalleryHotel} ${styles.toggleGalleryHotelTravelo}`}
                                                        >
                                                            <GalleryIcon />
                                                            {`+${photos.length} ${t("picture")}`}
                                                        </button>
                                                        <img src={photos[2].thumbnail} onContextMenu={(e) => e.preventDefault()} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => openGallery(6)}
                                            className={`${styles.mobileToggleGalleryHotel} ${process.env.THEME_NAME === "TRAVELO" && styles.toggleGalleryHotelTravelo}`}
                                        >
                                            {process.env.THEME_NAME === "TRAVELO" && <GalleryIcon />}
                                            {`+${photos.length} ${t("picture")}`}
                                        </button>
                                    </div>
                                    : (props.cipAirPortDetail.galleries && props.cipAirPortDetail.galleries.length > 2) ?
                                        <div className={styles.mosaicGallaryHotel}>
                                            <div className={styles.colMosaic}>
                                                <div className={styles.largeMosaic}>
                                                    <div className={styles.imageLargeMosaic} onClick={() => openGallery(0)}>
                                                        <img src={photos[0].thumbnail} onContextMenu={(e) => e.preventDefault()} />
                                                    </div>
                                                </div>
                                            </div>
                                            <div className={styles.colMosaic}>
                                                <div className={styles.smallMosaic}>
                                                    <div className={`${styles.imageSmallMosaic} ${styles.imageSmallMosaicThreeImage}`} onClick={() => openGallery(1)}>
                                                        <img src={photos[1].thumbnail} onContextMenu={(e) => e.preventDefault()} />
                                                    </div>
                                                </div>
                                                <div className={styles.smallMosaic}>
                                                    <div className={`${styles.imageSmallMosaic} ${styles.imageSmallMosaicThreeImage}`} onClick={() => openGallery(2)}>
                                                        <button
                                                            onClick={() => openGallery(2)}
                                                            className={`${styles.toggleGalleryHotel} ${styles.toggleGalleryHotelTravelo}`}
                                                        >
                                                            <GalleryIcon />
                                                            {`+${photos.length} ${t("picture")}`}
                                                        </button>
                                                        <img src={photos[2].thumbnail} onContextMenu={(e) => e.preventDefault()} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        : (props.cipAirPortDetail.galleries && props.cipAirPortDetail.galleries.length > 1) ?
                                            <div className={styles.mosaicGallaryHotel}>
                                                <div className={styles.colMosaic}>
                                                    <div className={styles.largeMosaic} onContextMenu={(e) => e.preventDefault()}>
                                                        <div className={styles.imageLargeMosaic} onClick={() => openGallery(0)}>
                                                            <img src={photos[0].thumbnail} />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className={`${styles.colMosaic} ${styles.colMosaicTwoColumn}`}>
                                                    <div className={styles.largeMosaic} onContextMenu={(e) => e.preventDefault()}>
                                                        <div className={styles.imageLargeMosaic} onClick={() => openGallery(1)}>
                                                            <img src={photos[1].thumbnail} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            : (props.cipAirPortDetail.galleries && props.cipAirPortDetail.galleries.length > 0) ?
                                                <div className={styles.onePicGalley}>
                                                    {process.env.THEME_NAME === "TAJAWAL" && topComment &&
                                                        <div className={styles.commentSmallMosaic}>
                                                            <span className={styles.commentSpan}>
                                                                {parse(topComment)}
                                                            </span>
                                                        </div>
                                                    }
                                                    <img src={photos[0].thumbnail} className={styles.picture} onContextMenu={(e) => e.preventDefault()} />
                                                    {
                                                        process.env.THEME_NAME === "TRAVELO" ? null :
                                                            (props.mapInfo.lat === 0) && (props.mapInfo.lang === 0) ?
                                                                null :
                                                                <>
                                                                    <div className={styles.imageXSmallMap} onClick={showModal}>
                                                                        <span className={styles.textViewMap}>{t('show-hotel-on-map')}</span>
                                                                        <img src="data:image/svg+xml;base64," />
                                                                    </div>
                                                                    <Modal
                                                                        title={`${props.hotelDetails.HotelCategoryName} ${props.hotelDetails.HotelName} ${props.hotelDetails.CityName} ${t('on-the-map')}`}
                                                                        open={visible}
                                                                        onCancel={handleCancel}
                                                                        footer={null}
                                                                    >
                                                                        <div className="mapModal">
                                                                            <MapWithNoSSR mapInfo={props.mapInfo} />
                                                                        </div>
                                                                    </Modal>
                                                                </>
                                                    }
                                                </div>
                                                :
                                                <div className={styles.onePicGalley} style={{ "backgroundImage": `url(${defaultBanner})` }}>
                                                    {process.env.THEME_NAME === "TAJAWAL" && topComment &&
                                                        <div className={styles.commentSmallMosaic}>
                                                            <span className={styles.commentSpan}>
                                                                {parse(topComment)}
                                                            </span>
                                                        </div>
                                                    }

                                                    {
                                                        process.env.THEME_NAME === "TRAVELO" ? null :
                                                            (props.mapInfo.lat === 0) && (props.mapInfo.lang === 0) ?
                                                                null :
                                                                <>
                                                                    <div className={styles.imageXSmallMap} onClick={showModal}>
                                                                        <span className={styles.textViewMap}>{t('show-hotel-on-map')}</span>
                                                                        <img src="data:image/svg+xml;base64," />
                                                                    </div>
                                                                    <Modal
                                                                        title={`${props.hotelDetails.HotelCategoryName} ${props.hotelDetails.HotelName} ${props.hotelDetails.CityName} ${t('on-the-map')}`}
                                                                        open={visible}
                                                                        onCancel={handleCancel}
                                                                        footer={null}
                                                                    >
                                                                        <div className="mapModal">
                                                                            <MapWithNoSSR mapInfo={props.mapInfo} />
                                                                        </div>
                                                                    </Modal>
                                                                </>
                                                    }
                                                </div>
                        }
                        <ReactBnbGallery
                            show={galleryOpened}
                            photos={photos}
                            onClose={closeGallery}
                            showThumbnails={true}
                            activePhotoIndex={currentImg}
                        />
                    </>}
            </div>
        </div>
    )
}

GalleryCipTravelo.getInitialProps = async () => ({
    namespacesRequired: ['common'],
})

GalleryCipTravelo.propTypes = {
    t: PropTypes.func.isRequired,
}
  
export default withTranslation('common')(GalleryCipTravelo)
