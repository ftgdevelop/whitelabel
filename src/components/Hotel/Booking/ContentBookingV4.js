...
const ContentBookingV4 = props => {
    ...
    return (
        <div className={styles.contentBooking}>
            {progress ?
                <div className={styles.statusMyBooking}>
                    ...
                    <div className={styles.proccessBooking}>
                        <h4>{reserveInfo && <>{t("hello")} {reserveInfo && reserveInfo.reserver.firstName} {reserveInfo && reserveInfo.reserver.lastName}</>  }</h4>
            ...
            :
                <>
                    {data && data.reserve.status === "Issued"?
                        <div className={styles.statusMyBooking}>
                            <div className={styles.successMyBooking}>
                                ...
                                <span> {progress? <LoadingOutlined style={{marginRight:'20px'}}/> : reserveInfo && reserveInfo.id} </span>
                                </div>
                            </div>
                            <div className={styles.contentMyBooking}>
                                <div className={styles.successBooking}>
                                    <CheckCircleIcon />
                                    <h4> {`سلام ${reserveInfo && reserveInfo.reserver.firstName} ${reserveInfo && reserveInfo.reserver.lastName}`} </h4>
                                    <span>{t("reserve-success")}</span>
                                    <span>
                                        {t("email-with-link2")}
                                        <b> {reserveInfo && reserveInfo.reserver.email} </b>
                                        {t("sent")}
                                        {t("for-tracking-with-us")}
                                        <b> {t("tracking-code")} : {reserveInfo && reserveInfo.id} </b>
                                        {t("use")}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className={styles.statusMyBooking}>
                            <div className={styles.errorMyBooking}>
                                ...
                                <div className={styles.reserveIdContent}>
                                    ...
                                    <span> {progress? <LoadingOutlined style={{marginRight:'20px'}}/> : reserveInfo.id} </span>
                                </div>
                    ...
                    <div className={styles.subjectRoomInfoMybooking}>
                        ...
                        <div>
                            <div className={styles.confirmationNoInfoBooking}>
                                <span> {t('tracking-code')}</span> {props.reserveInfo ? props.reserveInfo.id: <> <b> {t('checking')}</b> <Spin indicator={antIcon} /></>}
                            </div>
                        </div>
                    </div>
                    {props.reserveInfo && props.reserveInfo.rooms.map((item,index)=> {
                        ...
                        return( 
                            <div className={styles.contentRoomInfoMyBooking} key={index}>
                                <div className={styles.myRoomInfomation}>
                                    <div className={styles.headMyRoomInformation}>
                                        <UserIcon />
                                        {t('guest')} {item.passengers[0]?.firstName} {item.passengers[0]?.lastName}
                                    </div>
                                    <div className={styles.contentMyRoomInformation}>
                                        <div className="room-with-image">
                                            {item.image && <img src={item.image} className="room-image margin-end-10" alt=""/>}
                                            <div>
                                                ...
                                                {!!item.boardCode && board(item.boardCode)}
                                                {cancellation}                                                
                                            </div>
                                        </div>
                                    </div>
...
ContentBookingV4.getInitialProps = async () => ({
    namespacesRequired: ['common'],
})
  
ContentBookingV4.propTypes = {
