    ...
    return (
        <>
            ...
            <Modal
                ...
                className={`hotel-list-view-map ${i18n.language === "us" ?'ltr':'rtl'}`}
                footer={null}
            >
                            ...
                            <div className={styles.asideListView}>
                                <div className={styles.head}>
                                    <button onClick={() => setVisible(false)}>
                                        <RemoveOutlineIcon/>
                                        <span>{t('closeMap')} <small>(ESC)</small></span>
                                    </button>
                                </div>
                                {showHotelList &&
                                ...
                                <div className={styles.hotelListView}>
                                    ...
                                    {showHotelList &&
                                        <div className={styles.contentHotelListView}>
                                          ...
                                          {props.hotels.map(item=><div className={`${styles.hotelDetailMapView} ${hoveredId=== item.id? "isHovered":""}`} key={item.id} onContextMenu={(e)=> e.preventDefault()}>
                                            <Link as={`/fa/hotel-foreign/id=${item.id}/${props.searchedInfo}`} href={`/fa/hotel-foreign/id=${item.id}/${props.searchedInfo}`}>
                                              <a target="_blank" rel="noreferrer" title={item.name}
                                               onMouseEnter={()=>{markerHoverHandle(item.id);}}
                                               onMouseLeave={()=>{markerHoverHandle();}}
                                               >
                                                <img src={item.mainPhoto} alt={item.name} title={item.name} />
                                                    ...
                                                    <div className={styles.hotelPrice}>
                                                        <div>
                                                            <div className="new-price">
                                                                {item.salePrice ?
                                                                    <><span>{numberWithCommas(item.salePrice)}</span><span> ریال </span></>
                                                                    : <span style={{ fontSize: 12, color: '#e7412a' }}>قیمت وجود ندارد!</span>}
                                                            </div>
                                                        </div>
                                                        <button>{t('see-rooms')}</button>                                                       
                                                    </div>
                                                </div>
                                            </a>
                                            </Link> 
                                ...
                                hotelList={props.hotels.map(item => (
                                    {
                                        ...
                                        name: item.name ,
                                        rating:item.rating,
                                        url:`/fa/hotel-foreign/id=${item.id}`,
                                        boards:item.boards
