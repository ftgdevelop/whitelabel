...
class SelectHotel extends React.Component {
  ...
  setSelectedItem = (data) => {
    const selectedItem = data.find(
      (item) =>
        item.EntityId == this.props.selectedValueId ||
        item.id == this.props.selectedValueId,
    )
  ...
  onSelectHotel = (item) => {
    this.setState({
      selectedItem: item,
      input: item ? item.SearchValue || item.name : '',
      displayList: false,
    })
    ...
    return (
      <div ref={this.setWrapperRef} className={style.selectHotel}>
        ...
        >
          ...
          />
          {selectedItem || input ? (
            <button className="clear-input" onClick={this.onClearInput}>
                    ...
                    data.map((item, index) => (
                      ...
                      >
                        <div className="hotel-autocomplete-list-item-icon margin-end-5">
                          {item.EntityTypeId === 4 && <HotelOutlineIcon />}
                          {item.EntityTypeId === 3 && <LocationOutlineIcon />}
                          {item.EntityTypeId === 2 && <ProvinceOutlineIcon />}
                        </div>
                        <div>
                          ...
                          <div className={style.selectHotel__list_item_small}>
                            {item.SearchValue || item.parentName}
                          </div>
                        </div>
                      </li>
                    ))}
...
SelectHotel.getInitialProps = async () => ({
...
export default withTranslation('common')(SelectHotel)
