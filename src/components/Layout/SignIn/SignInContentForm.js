...
class SignInContentForm extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      activeTab: 0,
  ...
  onFinish = async (values) => {
    ...
    const username = values.userMobile
      .replace(/ +/g, '')
      .replace(/[{()}]/g, '')
      .replace(/-/g, '')
        ...
        <div className={style.signin}>
          ...
          <div className={styles.linkPage}>
            ...
            {!this.props.changeTab ? (
              ...
              <a onClick={this.props.changeStatus}>{t('create-account')}</a>
            )}
          </div>
        </div>
      </div>
...
SignInContentForm.getInitialProps = async () => ({
...
export default withRouter(
  withTranslation('common')(connect(null, mapDispatchToProps)(SignInContentForm)),
)
